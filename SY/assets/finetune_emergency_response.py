...
def training_function(script_args, training_args):
    ...
    eval_dataset = dataset.select(range(split, total_len))
    
    def create_conversation(sample):
        system_prompt = (
            "You are an expert first-aid assistant. "
            "Provide calm, essential, and actionable instructions for the user to handle this emergency safely. "
            "Focus on clear and practical first-aid steps that can be done immediately. "
            "Do not suggest calling emergency services; instead, concentrate on what the user can do right now."
            "IMPORTANT: Always begin your answer with this short disclaimer: 'I’m an AI assistant, not a medical expert. Here’s what to do now:'"
        )
        return {
            "messages": [
